...
* by Wei How Ng (828472)
* Final ish version I guess
*/
...
static void split_bucket2(InnerTable *table, int address, int indicator) {
	...
	// FIRST,
	// Check if we need to double the table by matching depth
	...
	//this is not filled with anything now
	bucket->full = false;
...
bool inner_table_lookup(InnerTable *itable, int64 key, int indicator) {
	assert(itable);
	
	// calculate inner table address for this key depending on indicator
	int index;
	if(indicator == 1){
		index = rightmostnbits(itable->depth, h1(key));
	}
	else{
		index = rightmostnbits(itable->depth, h2(key));
	}
	...
	if (itable->buckets[index]->full) {
		// found it?
		found = itable->buckets[index]->key == key;
	}
...
//------------------------------------------------
	...
	//call recursively to insert that key again -_-
...
bool xuckoo_hash_table_lookup(XuckooHashTable *table, int64 key) {
	assert(table);
	// 1 and 2 to be passed into sub-function to indicate hash function to use
	
	//if the key is found in table1 using the helper function, return true
	if(inner_table_lookup(table->table1, key, 1)){
		return true;
	}
	
	//if the key is found in table2 using the helper function, return true
	if(inner_table_lookup(table->table2, key, 2)){
		return true;
	}
...
void xuckoo_hash_table_stats(XuckooHashTable *table) {
	//soz didnt have time to do this :')
	return;
}
